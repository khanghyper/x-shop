...
const UpdateProductPage = ({categories, product}:any) => {
    console.log(product);
  ...
  <section class="max-w-4xl p-6 mx-auto">
      ...
      <form id="form">
          <div class="grid grid-cols-1 gap-6 mt-4">
              <div>
                  <label class="text-black">Cập nhật sản phẩm</label>
                  <input id="name" type="text" id="name" value="${product.name}"
              ...
              <div>
                  ...
                  <select id="categoryId" name="categoryId" 
                      class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                        ${categories.map((item:any) => `<option ${item.id === product.categoryId._id ? 'selected': ''} value="${item.id}">${item.name}</option>`)}
                  </select>
              </div>
              <div>
                  <label class="text-black dark:text-gray-200" for="passwordConfirmation">Số lượng</label>
                  <input id="instock" type="number" min="0" name="instock" value="${product.instock}"
                      class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
              </div>
              <div>
              ...
              <div>
                  ...
                  <input id="promotion" type="number" step="10" min="0" max="90" name="promotion" value="${product.promotion}"
                      class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
              </div>
              <div>
                  ...
                  <select id="isActive" name="isActive"
                      class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                      <option ${product.isActive ? 'selected': ''} value="true">Kích hoạt</option>
                      <option ${!product.isActive ? 'selected': ''} value="false">Tạm ngưng</option>
                  </select>
              </div>
              <div>
              ...
              <div>
                <img class="w-[200px] h-[200px] img-preview" src="${product.images.length ? product.images[0].url : ''}"/>
              </div>
          </div>
